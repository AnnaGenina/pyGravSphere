...
while program == True:
	...
	if option.strip() == "0":
		...
		if (yon.strip() == 'y') or (yon.strip() == 'Y') or (yon.strip() == 'yes'):
			workdir = input("Enter new name : ").strip()
			...
			for g in all_gals2:
				if g in gal_list:
					...
					while valid_galname == False:
						retype = input("This galaxy does not exist. Retype the name of this galaxy or leave blank: ")
						retype = retype.strip()
	...
	if option.strip() == "1":
		...
		project_name = input("Please name your new project: " )
		exists = os.path.isdir(workdir + '/' + project_name)
		if exists == True:
			print('This project already exists! Quitting!')
			break
		project_description = input("Please write a short summary (or long if you want): ")		
		...
		mpi_opt = input("1) Serial 	2) Multiprocessing  3) MPI (if you're running on a cluster) ")
		if mpi_opt == '3':
			num_cores = int(input('How many cores? '))
		...
		while valid_params == False:
			...
			plzh = input('1) PowerLaw or 2) Zhao ? ')
			...
			anis = input('1) Baes or 2) Constant ? ')
			...
			plummer = input('1) Plummer  2) ThreePlummers ')
		...
		if not sub_com:
			...
			sub_com = input("Enter script submission command or leave blank to run a single job on desktop: ") 
		...
		while valid_opt == False:
			mpi_opt = input("Running on a batch system y or n? " )
			if mpi_opt == 'y':
				...
				if not sub_script:
					...
					timevar = float(input("How much time do you need? (in hours) "))
					timevar = str(datetime.timedelta(hours = timevar))
					valid_opt = True
			elif mpi_opt == 'n':
				timevar = None	
		...
		opt_sub = input('Would you like to submit a job(s) now? y or n? ')
		if opt_sub.strip() == 'y':
			...
			if (mpi_opt == 'y') and (not sub_com) and (sub_script == True):
				print("You have not specified the submission command in the ", codedir,"/sub_command.txt file.")
				print("Specify the command now? (e.g. sbatch)")
				sub_com = input("Enter script submission command or leave blank to run on your desktop (single galaxy only): ") 
			...
			opt_gal = input("Which galaxies?  1) Specify    2) All ")
			if opt_gal.strip() == '1':
