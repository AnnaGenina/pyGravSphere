...
with open(workdir + "/" + project_name + "/Submissions/" + "script_bin_%s" %project_name + "_%d" %int(galaxy_number) +  ".py", "w") as f:
	f.write(r"""import os
import sys
import numpy as np
import matplotlib
...
""")
	f.write(r'sys.path.append("%s")' %workdir + '\n')
	f.write(r"""from GravSphere import gal_input
...
""")
	f.write(r"galaxy_number = %d" % int(galaxy_number) + "\n")
...
""")
	...
	for p in range(0, len(priors)):
		f.write(priors[p,0] + ',' + priors[p,1] + ' = ' + priors[p,2] + ',' + priors[p,3] + '\n')
...
#gRho0,gGamma0,gGamma1,gGamma2,gGamma3,gGamma4,gBeta0, gBeta1,gRa, gEta,gM1,gA1,gM2,gA2,gM3,gA3
	...
	if darkmatter == 'PL':
		params_dark =  'rho0,gamma0,gamma1,gamma2,gamma3,gamma4,'
		num_params = num_params + 6
		free_param_list = free_param_list + 'gRho0,gGamma0,gGamma1,gGamma2,gGamma3,gGamma4,'
	elif darkmatter == 'Zhao':
		params_dark = 'rhos,rs,alpha,beta,gamma,'
		num_params = num_params + 5
		free_param_list = free_param_list + 'gRho0, gRs0, gAlpha0,gBeta,gGamma0,'
	if anisotropy == 'Baes':
		params_anis = 'beta_t0,beta_t1, ra,eta,'
		num_params = num_params + 4
		free_param_list = free_param_list + 'gBeta0, gBeta1, gRa, gEta,'
	elif anisotropy	== 'Const':
		params_anis = 'beta_t0,'
		num_params = num_params + 1
		free_param_list = free_param_list + 'gBeta0,'
	if plummer == 'Plummer':
		params_plummer = ''
	elif plummer == 'Plummer3':
		params_plummer = 'm1,a1,m2,a2,m3,a3,'
		num_params = num_params + 6
		free_param_list = free_param_list + 'gM1,gA1,gM2,gA2,gM3,gA3,'
	if vsps == 'NOVSP':
		f.write("vsps = np.zeros((4,))" + '\n')

	params_mass = 'mstar'
	free_param_list = free_param_list + 'gMstar'
	num_params = num_params + 1
...
""")
	...
	if darkmatter == "PL":
		if plummer == 'Plummer':
			f.write("\t" + "like = simps.GetLikeBinAnisSphVSPPowerLawfunc(kindat[:,0],kindat[:,1],kindat[:,2],rho_param,beta_param,plum_param,vir_shape, 10**mstar, r_c,bins, min_rad, max_rad)" + '\n')
		else:
			f.write("\t" + "like = simps.GetLikeBinAnisSphVSPPowerLawfunc(kindat[:,0],kindat[:,1],kindat[:,2],rho_param,beta_param,plum_param,vir_shape, 10**mstar, r_c,bins,min_rad, max_rad)" + '\n')
	else:
		if plummer == 'Plummer':
			f.write("\t" + "like = simps.GetLikeBinAnisSphVSPfunc(kindat[:,0],kindat[:,1],kindat[:,2],rho_param,beta_param,plum_param,vir_shape, 10**mstar, r_c,bins, min_rad, max_rad)" + "\n")
		else:
			f.write("\t" + "like = simps.GetLikeBinAnisSphVSPfunc(kindat[:,0],kindat[:,1],kindat[:,2],rho_param,beta_param,plum_param,vir_shape, 10**mstar, r_c,bins, min_rad, max_rad)" + "\n")
...
#and (gamma0 < gamma1 < gamma2) and (gamma2 < gamma3 < gamma4) 

	if darkmatter == 'PL':
		prior_dm = "(rho0min < rho0 < rho0max) and all(minarr < thetau < maxarr for minarr,thetau,maxarr in zip(min_gammas,gammas,max_gammas)) "

#(gamma0min < gamma0 < gamma0max) and (gamma1min < gamma1 < gamma1max) and (gamma2min < gamma2 < gamma2max) and (gamma3min < gamma3 < gamma3max) and (gamma4min < gamma4 < gamma4max) and (bigger == 0) "
	elif darkmatter == 'Zhao':
...
""")
	...
	f.write("\t" + """nll = lambda *args: -lnlike(*args)

	gRho0 = 8.
	gRs0 = np.log10(r_c) + 0.1
	gGamma0 = 1.0
	gGamma1 = 1.5
	gGamma2 = 2.0
	gGamma3 = 2.5
	...
	gRa = np.log10(r_c)
	gEta = 1.5
	gM1 = np.log10(lightpower[0])
	gM2 = np.log10(lightpower[1])
	gM3 = np.log10(lightpower[2])
	gMstar = np.log10(stellar_mass)
	gA1 = lightpower[3]
	gA2 = lightpower[4]
	gA3 = lightpower[5]""")

	f.write("\n" + "\t" + "free_param_list = (%s)" %free_param_list + "\n")
	...
	try:
		pos = np.loadtxt(workdir  + project_name + '/Galaxy_%d' % galaxy_number +'/Galaxy_%d_LastWalkerPos' % galaxy_number + project_name + ".txt" )
		print 'Got pos'
		...
		chains = np.genfromtxt(workdir + '/' + project_name + '/Galaxy_%d' % galaxy_number +'/Galaxy_%d_' %galaxy_number + "Chains" + project_name + ".txt")
		...
		try:
			...
			f_chains = open(workdir + '/' + project_name + '/Galaxy_%d' % galaxy_number +'/Galaxy_%d_' %galaxy_number + "Chains" + project_name + ".txt", "w")
			f_chains.close()
	...
	for i in range(0, (tot_iter)/100):
		...
		if completed + i*100 > burn_in:
			...
			f_handle = file(workdir + '/' + project_name + '/Galaxy_%d' % galaxy_number +'/Galaxy_%d_Chains' %galaxy_number + project_name + ".txt", 'a+')
	...
	np.savetxt(workdir + '/' + project_name + '/Galaxy_%d' % galaxy_number + '/Galaxy_%d_LastWalkerPos' % galaxy_number + project_name + ".txt" , pos)
...
t = time.time()
mpi_time = os.system("mpiexec -n %d" %(cores) +  " python " + workdir + "/" + project_name + "/Submissions/" + "script_bin_%s" %project_name + "_%d" %int(galaxy_number) +  ".py")
